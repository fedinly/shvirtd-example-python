...
def index(request: Request, ip_address: Optional[str] = Depends(get_client_ip)):
    ...
    try:
        with get_db_connection() as db:
            cursor = db.cursor()
            query = "INSERT INTO {db_table_name} (request_date, request_ip) VALUES (%s, %s)"
...
def get_requests():
    ...
    try:
        with get_db_connection() as db:
            cursor = db.cursor()
            query = "SELECT id, request_date, request_ip FROM {db_table_name} ORDER BY id DESC LIMIT 50"
